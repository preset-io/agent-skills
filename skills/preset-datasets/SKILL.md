...
Do not create or update databases, datasets, columns, metrics, imports, exports, schema refreshes, uploads, database tests, SQL validation, or SQL Lab queries from this skill without explicit confirmation. Route database connection workflows through `preset-database-connections`. For dataset or database mutations, summarize the target workspace, database/dataset IDs, request body or SQL, and expected effect before making the call.
