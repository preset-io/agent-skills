...
4. Before any `POST`, `PUT`, `PATCH`, `DELETE`, audit download, role change, invite, workspace lifecycle action, or member removal, also load [../preset-api/references/safety-policy.md](../preset-api/references/safety-policy.md) and get explicit user confirmation.
5. If the requested workflow is not in the focused references, check [references/deferrals.md](references/deferrals.md) before improvising from adjacent Manager endpoints.
