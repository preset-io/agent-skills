...
5. **Safety gate** — before any confirmation-gated operation (mutations, imports, credential reads, unclassified SQL, broad exports), load [`preset-api/references/safety-policy.md`](skills/preset-api/references/safety-policy.md), summarize target + payload + effect, and get explicit user confirmation. Reads run directly per the gate policy tiers.
